angular.module("ciclotourApp").controller('PerformedRoutesController', function($scope, RoutesAPI, Message){
    $scope.routes = [];
    $scope.next = null;
    ...
    function get_routes(){
        if($scope.next == null && $scope.routes.length == 0)
            RoutesAPI.get_performed_routes().success(getRoutesSuccess).error(getRoutesFail);
        else if($scope.next && $scope.routes.length > 0)
            RoutesAPI.get_next_performed_routes($scope.next).success(getRoutesSuccess).error(getRoutesFail);
    }
